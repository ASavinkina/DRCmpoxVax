...
# Read in demographic data from the DRC
demographics <- read.csv("/Users/alexandrasavinkina/Documents/Research/DRC Mpox/DRCmpoxVax/Demographic_data_2.4.24.csv")

#Create an output table to feed model output into
...
# Set up vaccination strategies for analysis
...
# Beginning for-loop which will loop through all vaccination scenarios 

for (p in 1:nrow(output_all)) {


  # Set up results table to collect data for each vaccination strategu  
...
colnames(results_all) <- c( "time","S_u5","I_u5","V_u5","D_u5",            
    ...
    # Set vaccination effectiveness
    # 
    vax_effect <- 0.75 #input$vax_effect
  
    
    
    #set proportion of infections that are sylvatic
  ...
  # for loop that loops through each province 
    
 for (i in 1:nrow(demographics)) {
   
   # set number of people vaccinated per vaccination scenario 
   # 
   ...
   # set province based on province being run at the time
      province = demographics[i,1]
      
      # assign model parameters
      
      exogshock_u5= ((demographics[i,9]*prop_sylvatic)*(1-prop_sylvatic_adult-prop_sylvatic_u15))/demographics[i,2]
      exogshock_u15= ((demographics[i,9]*prop_sylvatic)*prop_sylvatic_u15)/demographics[i,3]
      exogshock_o15= ((demographics[i,9]*prop_sylvatic)*prop_sylvatic_adult)/demographics[i,4]
      ...
      u15_pop = as.numeric(demographics[i,3]) # proportion of pop over 5 and under 15
      o15_pop = as.numeric(demographics[i,4]) # proportion of pop over 15
      propHR = 0.05#0.05#0.05 #input$propHR #as.numeric(demographics[i,5])
      ...
      mortality_u15 = 0.045 #input$cfr_u15 #0.09 # mortality rate under 15
      ...
      # This separates out vaccination scnearios by endemic, historic, and all
      ...
      # Assign initial conditions. Overall: everyone starts Susceptible, unless there is an initial infection (not in this analysis)
      # or someone level of vaccination
        ...
        yinit = c(
          ...
          #Commented out lines are for sensitivity analysis looking at 21% immunity among adults
           # S_o15LR = as.integer(o15_pop*(1-propHR)*(1-0.21)-initial_inf_o15LR-(o15_pop*(1-propHR)*vax_S_o15LR)*(1-0.21)),
          I_o15LR = as.integer(initial_inf_o15LR), 
          V_o15LR= as.integer(o15_pop*(1-propHR)*vax_S_o15LR), 
          #     V_o15LR= as.integer(o15_pop*(1-propHR)*vax_S_o15LR)*(1-0.21)+as.integer(o15_pop*(1-propHR)*(0.21)),
          D_o15LR=0, 
          R_o15LR=0,
          S_o15HR = (as.integer(o15_pop*propHR-initial_inf_o15HR-(o15_pop*propHR*vax_S_o15HR))),
          #    S_o15HR = as.integer(o15_pop*(propHR)*(1-0.21)-initial_inf_o15HR-(o15_pop*(propHR)*vax_S_o15HR)*(1-0.21)),
          I_o15HR = as.integer(initial_inf_o15HR), 
          V_o15HR= as.integer(o15_pop*propHR*vax_S_o15HR),
          #      V_o15HR= as.integer(o15_pop*propHR*vax_S_o15HR)*(1-0.21)+as.integer(o15_pop*(propHR)*(0.21)),
          D_o15HR=0 , 
          R_o15HR=0
        )
      
      # this creates the modeling function, solving through the equations. They are separated by age group  
      ...
      vac_model <- function(times,yinit,pars){
        ...
        with(as.list(c(yinit,pars)), {
          ...
          dS_u5 <- -beta_u5u5*S_u5*(I_u5/(N_u5+N_u15+N_o15LR+N_o15HR)) -
            ...
            S_u5*(theta_u5-theta_u5*(V_u5/N_u5))
          ...
          dS_u15 <- -beta_u15u5*S_u15*(I_u5/(N_u5+N_u15+N_o15LR+N_o15HR)) -
            ...
            S_u15*(theta_u15-theta_u15*(V_u15/N_u15))
          ...
          dS_o15LR <- -beta_o15u5*S_o15LR*(I_u5/(N_u5+N_u15+N_o15LR+N_o15HR)) -
            ...
            S_o15LR*(theta_o15-theta_o15*(V_o15LR/N_o15LR)) 
          ...
          dS_o15HR <- -beta_o15u5*S_o15HR*(I_u5/(N_u5+N_u15+N_o15LR+N_o15HR)) -
            ...
            beta_o15o15HR*S_o15HR*(I_o15HR/N_o15HR)-
            S_o15HR*(theta_o15-theta_o15*(V_o15HR/N_o15HR)) 
      ...
      # Setting parameters used in the model function above 
      pars = list(
        ...
        theta_o15= exograte_o15, #rate of exogenous shocks
    ...
    # prints results to save by province
  ...
  #dataset for final deaths statistics
    ...
    # Summarizes data across the tables
...
# Formatting for printing out
output_all1 <- t(apply(output_all, 1, unlist))

output_all1 <- as.data.frame(output_all1)

# Writes final data as csv for tables and figures

#write.csv(output_all1, file="/Users/alexandrasavinkina/Documents/Research/DRC Mpox/DRCmpoxVax/results/vacciation_results_72824.csv")
#write.csv(output_all1, file="/Users/alexandrasavinkina/Documents/Research/DRC Mpox/DRCmpoxVax/results/SA/vacciation_results_highHR_72824.csv")
